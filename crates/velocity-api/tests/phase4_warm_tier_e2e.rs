...
use velocity_api::tiering::{TierWindows, TieredEventReader, WarmEventReader};
...
fn build_app_state(pool: PgPool, schemas: Arc<SchemaRegistry>, warm: &WarmHarness) -> AppState {
    ...
    AppState::new(schemas, pool).with_tiering(tiered, cold_jobs)
}
...
async fn cold_at_returns_202_with_job_id() {
    ...
    let cold_jobs = velocity_api::tiering::cold_stub::ColdJobStore::new();
    let app_state = AppState::new(Arc::clone(&schemas), pool).with_tiering(tiered, cold_jobs);
    let app = router::build(app_state).layer(from_fn(inject_identity(ident())));
