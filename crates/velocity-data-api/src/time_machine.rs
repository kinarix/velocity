...
//!   ([`velocity_core::row_filter::payload_visible`]). One decision per
...
//!   [`velocity_core::field_filter::FieldFilterIndex::strip_for_read`]; every
//!   `diff` runs through
//!   [`velocity_core::field_filter::FieldFilterIndex::strip_diff_for_read`]
...
use velocity_core::handler_util::{identity_from_ext, resolve_schema, SchemaPathParts};
use velocity_core::rbac::{self, op};
use velocity_core::registry::registry_key;
...
async fn ensure_entity_visible(
    pool: &sqlx::PgPool,
    schema: &velocity_core::registry::ResolvedSchema,
    schema_org: &str,
    entity_uuid: uuid::Uuid,
    identity: &Identity,
) -> Result<(), ApiError> {
    ...
    if !velocity_core::row_filter::payload_visible(schema, identity, &payload)? {
        return Err(ApiError::NotFound);
    }
    Ok(())
}
...
pub async fn history(
    ...
    _decision_ext: Option<Extension<velocity_core::auth::AuthDecision>>,
    identity: Option<Extension<Identity>>,
    Path(parts): Path<EntityPathParts>,
    Query(q): Query<HistoryQuery>,
) -> Result<(StatusCode, Json<Value>), ApiError> {
    let (schema_parts, entity_id) = split_entity(parts);
    let schema = resolve_schema(&state.registry, schema_parts)?;
...
pub async fn snapshot(
    ...
    for row in rows {
        ...
        let visible = velocity_core::row_filter::payload_visible(row_schema, &identity, &gate)?;
        if !visible {
            continue;
        }
