...
//!
//! Scope vs the simpler [`velocity_core::query`] list builder:
//!
...
use velocity_core::cursor::CursorSigner;
use velocity_core::error::ApiError;
use velocity_core::identity::Identity;
use velocity_core::query::{DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE};
use velocity_core::rbac::{check_access, op};
use velocity_core::registry::{registry_key, ResolvedSchema, SchemaRegistry};
...
/// Extension trait adding envelope (de)serialisation to the core
/// [`velocity_core::cursor::CursorSigner`]. Defined here as a local trait
/// (not inherent methods) because the signer type lives in the core crate;
/// an extension trait is the orphan-rule-safe way to attach DSL-specific
/// behaviour. The raw sign/verify primitive lives in core so non-DSL callers
/// can share the HMAC key without linking this module.
trait CursorCodec {
    fn encode(&self, env: &CursorEnvelope) -> Result<String, ApiError>;
    fn decode(&self, cursor: &str) -> Result<CursorEnvelope, ApiError>;
}

impl CursorCodec for CursorSigner {
    fn encode(&self, env: &CursorEnvelope) -> Result<String, ApiError> {
        let payload =
