...
use velocity_core::field_filter::FieldFilterIndex;
...
use velocity_core::validate::{validate_fields, validate_rules, WriteMode};
...
pub async fn get_one(
    ...
    let schema = resolve_schema(&state.registry,(org, app, domain, object, version))?;
    let identity = identity_from_ext(identity_ext);
    let decision = decision_ext.map(|Extension(d)| d);
    let request_id = request_id_from_headers(&headers);
    audit_if_denied(
