...
use velocity_core::query::{build_list, ListQuery};
...
async fn crud_round_trip() {
    ...
    .await;
    assert!(matches!(stale, Err(velocity_core::ApiError::VersionConflict)));

    // DELETE — soft
    handler_delete(&api_pool, &schema, &identity, &id).await.unwrap();
    let missing = handler_get(&api_pool, &schema, &identity, &id).await;
    assert!(matches!(missing, Err(velocity_core::ApiError::NotFound)));
...
async fn handler_create(
    ...
    identity: &Identity,
    payload: &Value,
) -> Result<Value, velocity_core::ApiError> {
...
async fn handler_get(
    ...
    .await?;
    row.map(|r| sqlx::Row::get::<Value, _>(&r, "row")).ok_or(velocity_core::ApiError::NotFound)
}
...
async fn handler_list(
    ...
    identity: &Identity,
    q: &ListQuery,
) -> Result<Vec<Value>, velocity_core::ApiError> {
...
async fn handler_update(
    ...
    match outcome {
        Ok(Some(row)) => Ok(row),
        Ok(None) => Err(velocity_core::ApiError::NotFound),
        Err(sqlx::Error::RowNotFound) => Err(velocity_core::ApiError::NotFound),
        Err(sqlx::Error::Protocol(msg)) if msg == "__version_conflict__" => {
            Err(velocity_core::ApiError::VersionConflict)
        }
        Err(e) => Err(velocity_core::ApiError::Database(e)),
    }
}
