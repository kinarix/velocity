...
pub struct SchemaDefinitionStatus {
    ...
    /// The Typesense concrete collection name that the schema's alias
    /// currently points at. Surfaces in `kubectl describe sd` so an
    /// SRE can answer "which underlying collection is serving search
    /// right now" without an out-of-band `GET /aliases` call. Only
    /// populated for Tier-3 schemas.
    #[serde(default, skip_serializing_if = "Option::is_none")]
