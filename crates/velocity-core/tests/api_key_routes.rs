...
use velocity_core::auth::api_key::{ApiKeyChecker, ApiKeyError, ApiKeyRecord, ApiKeyScope};
use velocity_core::auth::{authenticate, AuthRegistry, AuthState, JwksCache, ResolvedAuthStrategy};
...
impl ApiKeyChecker for MockChecker {
    async fn lookup(&self, plaintext: &str) -> Result<ApiKeyRecord, ApiKeyError> {
        ...
        velocity_core::auth::api_key::validate_plaintext(plaintext)?;
