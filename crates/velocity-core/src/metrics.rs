...
/// Phase 12b: gauge that is `1` while the API runs with the anonymous auth
/// bypass active, `0` otherwise. Set once at startup. A non-zero value here
/// is a hard signal that authentication is disabled — alert on it in any
/// non-test environment.
pub fn auth_anonymous_mode() -> &'static IntGauge {
    static GAUGE: OnceLock<IntGauge> = OnceLock::new();
    GAUGE.get_or_init(|| {
        #[allow(clippy::expect_used)]
        let g = IntGauge::new(
            "velocity_auth_anonymous_mode",
            "1 when the data-plane auth bypass (VELOCITY_API_AUTH_MODE=anonymous) is active, else 0",
        )
        .expect("constructing anonymous-mode gauge (static definition)");
        #[allow(clippy::expect_used)]
        registry()
            .register(Box::new(g.clone()))
            .expect("registering anonymous-mode gauge (must be unique)");
        g
    })
}
