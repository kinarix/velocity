...
/// Emit a single loud WARN the first time an anonymous-mode request is
/// admitted. Per-request logging would flood; the ongoing signal is the
/// `velocity_auth_anonymous_mode` gauge and the `/readyz` banner.
fn warn_anonymous_mode_once() {
    use std::sync::atomic::{AtomicBool, Ordering};
    static WARNED: AtomicBool = AtomicBool::new(false);
    if !WARNED.swap(true, Ordering::Relaxed) {
        tracing::warn!(
            "AUTH BYPASS ACTIVE — VELOCITY_API_AUTH_MODE=anonymous: data-plane requests \
             are admitted without authentication as actor=anonymous. This is a test mode; \
...
pub struct AuthState {
    ...
    /// Phase 12b: data-plane auth mode. When `Anonymous`, `try_authenticate`
    /// injects a fixed anonymous identity and skips credential verification
    /// for `/api/{schema}` routes. Default `Enforced`.
...
impl AuthState {
    ...
    /// Builder-style — set the data-plane auth mode (Phase 12b). When
    /// `Anonymous`, data-plane requests bypass credential verification and
    /// run as the anonymous identity. Default `Enforced`.
...
async fn try_authenticate(state: &AuthState, req: &mut Request<Body>) -> Result<(), ApiError> {
    ...
    // Phase 12b: anonymous bypass. When the platform runs in anonymous mode,
    // data-plane requests skip all credential verification and run as the
    // fixed anonymous identity. This is a bypass, not a removal: the request
    // still flows through audit (actor = "anonymous") and RLS context
    // (`SET LOCAL app.current_user = 'anonymous'`), so ADR-005 / ADR-007 stay
    // intact. We do NOT resolve the schema's AuthStrategy here — the point is
    // to exercise the platform before any AuthStrategy exists. Inter-service
    // tokens (`/api/platform/*`, warm-reader) are unaffected: those routes
    // never reach this middleware (they have <5 path segments) and check
    // their own bearer tokens.
    if state.auth_mode == AuthMode::Anonymous {
        warn_anonymous_mode_once();
...
fn schema_path_from_uri(uri_path: &str) -> Option<SchemaPath> {
    let mut segments: Vec<&str> = uri_path.trim_start_matches('/').split('/').collect();
    // `velocity-search` mounts schema routes under a `/search` prefix
    // (Phase 12a). Strip it so per-schema read RBAC resolves the same schema
    // as the data tier.
    if segments.first() == Some(&"search") {
        segments.remove(0);
    }
...
mod tests {
    ...
    #[test]
    fn schema_path_from_uri_strips_search_prefix() {
        // velocity-search mounts schema search under `/search` — per-schema
        // read RBAC must still resolve the schema.
        let p = schema_path_from_uri(
            "/search/api/acme/supply-chain/procurement/purchase-order/v1/search",
        )
        .unwrap();
        assert_eq!(p.org, "acme");
        assert_eq!(p.object, "purchase-order");
        assert_eq!(p.version, "v1");
        // cross-org search (`/search/api/{org}/search`) is org-wide, not a
        // single schema → None (the handler does its own per-schema filtering).
        assert!(schema_path_from_uri("/search/api/acme/search").is_none());
    }
    ...
    fn req_to(uri: &str) -> Request<Body> {
        let mut req = Request::new(Body::empty());
        *req.uri_mut() = uri.parse().unwrap();
        req
    }

    // ── Phase 12b: anonymous-mode bypass ─────────────────────────────────

    #[tokio::test]
    async fn anonymous_mode_admits_request_and_injects_anonymous_identity() {
        // No AuthStrategy registered — proves the bypass runs *before*
        // strategy resolution, which is the point: exercise the platform
        // before any AuthStrategy exists.
        let state = state_with_strategies(vec![]).with_auth_mode(AuthMode::Anonymous);
        let mut req = req_to("/api/acme/supply-chain/procurement/purchase-order/v1");
        try_authenticate(&state, &mut req).await.expect("anonymous mode must admit the request");
        let id = req.extensions().get::<Identity>().expect("identity injected");
        assert!(id.is_anonymous(), "injected identity must be anonymous");
    }

    #[tokio::test]
    async fn enforced_mode_rejects_same_request_when_schema_absent() {
        // Counterpart: with enforcement on (the default) and no schema in the
        // registry, the identical request is rejected — proving the bypass is
        // what changes behaviour, not the empty registry.
        let state = state_with_strategies(vec![]);
        let mut req = req_to("/api/acme/supply-chain/procurement/purchase-order/v1");
        let err = try_authenticate(&state, &mut req).await.unwrap_err();
        assert!(matches!(err, ApiError::SchemaNotFound), "expected SchemaNotFound, got {err:?}");
    }

    #[tokio::test]
    async fn anonymous_mode_leaves_non_api_routes_untouched() {
        // Non-schema routes return Ok with no identity in either mode — the
        // bypass must not start injecting identities onto /healthz etc.
        let state = state_with_strategies(vec![]).with_auth_mode(AuthMode::Anonymous);
        let mut req = req_to("/healthz");
        try_authenticate(&state, &mut req).await.expect("non-api route passes through");
